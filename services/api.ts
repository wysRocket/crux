...
const getSignedUrl = async (body: any) => {
  try {
    const response = await axios.post(
      `https://d37729g2srnlx.cloudfront.net/prod/getPreSignUrl`,
      body,
      {
        headers: {
...
const fileUploadHandler = async (file: any, userId: string) => {
  try {
    const body = {
...
const addFileToDB = async (
  ...
  try {
    ...
    const response = await axios.post(
      `https://d37729g2srnlx.cloudfront.net/prod/updateDb`,
      body,
      {
        headers: {
          "x-api-key": "Dh2j2oj3kd7FiTmEaMJan9oi1aA9DfcAaxL0qElv",
...
const getFilesList = async (userId: string) => {
  try {
    const response = await axios.get(
      `https://d37729g2srnlx.cloudfront.net/prod/updateDb?userId=${userId}`,
      {
        headers: {
          "x-api-key": "Dh2j2oj3kd7FiTmEaMJan9oi1aA9DfcAaxL0qElv",
        },
    ...
    if (response?.data) {
      return response.data.map((eachData: any) => {
        console.log(
          eachData["lastUpdatedAt"],
          dayjs(eachData["lastUpdatedAt"]).format("LLL")
        );
        return {
          id: eachData["bb-pk"],
...
const deleteEntry = async (fileKey: string, bbkey: string) => {
  try {
    const response = await axios.delete(
      `https://d37729g2srnlx.cloudfront.net/prod/updateDb?fileKey=${encodeURIComponent(
        fileKey
      )}&bbKey=${bbkey}`,
      {
        headers: {
